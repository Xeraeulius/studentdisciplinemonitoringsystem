	    ...
	    -->
	    	<form action="update_dresscode_violation.php" method="POST">
			    <div class="box-body table-responsive no-padding">
					<table class="table table-hover">
						...
						<?php 
							global $conn;
							$select = "SELECT name, active, gender, violation_code from violation_code where gender = 'MALE' or gender = 'EVERYONE' order by active asc, gender asc";
							$result = $conn->query($select);

							while ($row = $result->fetch_object()) {
								if ($row->gender === 'MALE' or $row->gender === 'EVERYONE') {
						?><tr>
						        <td><label><?php echo $violation_name = $row->name; ?></label></td>
						        <td><p><?php echo $row->active; ?></p></td>
				    			<td><?php echo $row->gender; ?></td>
			    				<td><button type="submit" name="update_violation_list_male" value="<?php echo $row->violation_code ?>" class="btn btn-primary">Update</button></td>
				    		<?php
				    			}
				    		?>
					      </tr>
			...
			<form action="add_dresscode_violation.php" method="POST">
		        <div class="box-footer clearfix">
		      		<button type="submit" name="btn_add_male" class="btn btn-default pull-left"><i class="fa fa-plus"></i> Add</button>
		        </div>
			</form>
		</div>
	</div>
