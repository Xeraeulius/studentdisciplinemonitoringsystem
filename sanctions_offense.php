...
if (isset($_POST['sanctions_minor'])) {
	global $conn;

	$id = $_POST['sanctions_minor'];

	$select = "SELECT reference_number, title from offense_details where (status = 'Active' AND type = 'Minor Offense') AND id = '$id'";
	...
	$select_join = "SELECT 		offense_details.description
					...
					WHERE	  	offense_details.id = $id AND type = 'Minor Offense'";
