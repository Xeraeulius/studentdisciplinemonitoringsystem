...
create_minor_offense();
update_minor_offense()
?>
...
<div class="alert alert-success alert-dismissable">
	...
	<p><i class="icon fa fa-check"></i> Minor Offense Detail Updated</p>
</div>
<?php	
}
?>
...
<div class="alert alert-success alert-dismissable">
	<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
	<p><i class="icon fa fa-check"></i> Minor Offense Detail Added:</p>
	<ul>
	    ...
	    -->
	    	<form action="update_offense_details.php" method="POST">
			    <div class="box-body table-responsive no-padding">
					<table class="table table-hover">
						...
						<?php 
							global $conn;
							$select = "SELECT id, reference_number, description, title, status from offense_details where type = 'Minor Offense'";
							$result = $conn->query($select);

							while ($row = $result->fetch_object()) {
								if ($row->type = "Minor Offense") {
						?><tr>
			    				...
			    				<td style="width: 150px;"><button type="submit" name="update_minor_offense" value="<?php echo $row->id ?>" class="btn btn-primary">Update</button></td>
				    		<?php
				    			}
				    		?>
					      </tr>
			...
			<form action="add_offense_details.php" method="POST">
		        <div class="box-footer clearfix">
		      		<button type="submit" name="btn_add_minor" class="btn btn-default pull-left"><i class="fa fa-plus"></i> Add</button>
		        </div>
			</form>
		</div>
	</div>
