  ...
  <div class="row">
    ...
    <div class="col-md-9">
      <div class="box box-primary">
        ...
        <div class="box-body">
        <form action="major_offense_list.php" method="POST">
        ...
        <label>Name of Student:</label>
          ...
          ?>
              <input class="form-control" disabled value="<?php echo $row->last_name . ", " . $row->first_name . " " . $row->middle_name; ?>">
          <?php
            }
          ?>
        </div>
        ...
        <div class="form-group">
          <label>Reference Number: </label>
              <?php
                global $conn;
                $select = "SELECT id, reference_number, description, title FROM offense_details WHERE (status = 'Active' AND type = 'Major Offense') AND id = '$sanctions_id'";
                $result = $conn->query($select);
                while ($row = $result->fetch_object()) {
        ...
        <div class="box-footer">
          <button type="submit" name="btn_major" value="<?php echo $student_id ?>" class="btn btn-primary pull-right">Submit</button>
        </div>
        </form>
        </div><!-- /.box-body -->
      </div><!-- /.col -->
